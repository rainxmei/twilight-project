...
class Player {
    public:
        ...
        // fungsi ketika memory Player dihapus
        ~Player() {
            cout << "player" << Player::nama << " dihapus" << endl;
        }
};
...
Player createStackPlayer(){
    Player stackPlayer = Player("stack di buat");
    return stackPlayer;
}

Player* createHeapPlayer(){
    Player* heapPlayer = new Player("heap di buat");
    return heapPlayer;
}
...
int main(int argc, char const *argv[])
{
    ...
    cout << (*playerPointer1).nama << endl; // tidak bekerja
    ...
    // cout << playerPointer2->nama << endl; // memory free
